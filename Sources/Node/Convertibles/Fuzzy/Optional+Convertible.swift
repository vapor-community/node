extension Optional: NodeConvertible {
    public init(node: Node) throws {
        guard Wrapped.self is NodeInitializable.Type else {
            throw NodeError.invalidContainer(container: "\(Optional.self)", element: "\(Wrapped.self)")
        }
    ...
    public func makeNode(in context: Context?) throws -> Node {
        ...
        guard let representable = value as? NodeRepresentable else {
            throw NodeError.invalidContainer(container: "\(Optional.self)", element: "\(String(describing: value))")
        }
